...
public class AssignColour : MonoBehaviour
{
    ...
    private List<ColorRules> cRules;
    ...
    void Start()
    {
        ...
        cRules = new List<ColorRules>();
        mRules = new List<MovementRules>();
    }

    //not in start because other script needs to finish first
    public void SetRules(List<MovementRules> mr, List<ColorRules> cr)
    {
        mRules = mr;
        cRules = cr;

        foreach (ColorRules rule in cRules)
        {
